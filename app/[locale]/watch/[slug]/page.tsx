...
import { watchPosts, findEditorial, localizedField } from "@/data/editorial";
import { EditorialDetail } from "@/components/editorial/EditorialDetail";
import type { Locale } from "@/i18n/routing";
import type { Metadata } from "next";

export function generateStaticParams() {
  return watchPosts.map((p) => ({ slug: p.slug }));
}
...
export async function generateMetadata({
  ...
  const p = findEditorial(slug, "media_review");
  if (!p) return {};
  return {
...
export default async function WatchPostPage({
  params,
}: { params: Promise<{ locale: Locale; slug: string }> }) {
  const { locale, slug } = await params;
  setRequestLocale(locale);
  const post = findEditorial(slug, "media_review");
  if (!post) notFound();
